...
*Last updated: 2026-07-06 (through Cycle 29 merged — v0.29.0 — Monitor→Librarian pipeline + HITL fix + Scout gate; Cycle 28.1 write-wall hotfix merged on top — v0.28.1's identity chokepoint work landed via a separate branch in parallel, see `docs/cycle-28.1-hotfix.md`)*
...
Current implemented surface (through Cycle 29, v0.29.0, plus the Cycle 28.1 write-wall hotfix merged on top, v0.28.1):
...
- **Calendar-write arm (Cycle 26):** kage's first WRITE arm — EventKit create-only (write-only access), `propose → approve → execute` over human-readable markdown proposals + `kage calendar` CLI; excluded from `_detect_arms`, HITL-gated, audited. The template for future write arms; delete/reschedule/undo deferred (need full-access via a signed-helper identity)
- **Two-pass privacy gate (Cycle 27):** `gate.two_pass_gate()` across all 11 egress sites; vault-value silent redaction (Pass 1) + generic PII detection with allowlist/queue (Pass 2); `kage allow` + `kage privacy review` CLI
- **Identity registry + account-scoped arms (Cycle 28):** `identity.py` CRUD + `kage identity` CLI; read-only identity class blocks write-permission arms; per-identity arm overrides (e.g. account-scoped Gmail/Calendar routing); identity groups (shared memory pool, distinct arm routing)
- **Write-wall hotfix (Cycle 28.1):** `identity.resolve_write_identity()` — the single chokepoint every writer to `memory_identities` now routes through (previously only `cli._save`'s CLI callers had it; MCP `kage_remember` and Librarian `write_note` could tag a note with a raw label instead of its canonical group, making it permanently unreachable). `kage identity repair` migrates any notes tagged before the fix; `kage doctor` gained an "identity tags canonical" check that fails loudly, not silently, on registry corruption. See `docs/cycle-28.1-hotfix.md`.
...
**COLD REVIEW (HARD RULE — never skip, every cycle):** Every major artifact gets **1-2 cold-review passes before it is accepted** — the **pitch** (before any code is written), the **code**, and the **tests**. "Cold" = a fresh, adversarial pass hunting for contradictions and load-bearing flaws, NOT a re-read by the same train of thought that produced it. Each pass names what it checked against (architecture, the 10 characteristics, jugaad, ponytail, completeness, security/egress) and the count is tracked in the doc/PR (e.g. "v3, two cold reviews"). **Mechanism:** the **pitch** is authored by cloud, so its cold review must come from an *independent* context — spawn a **subagent** (or fresh session) to review it against the real code seams. **Code and tests** are authored by local (Qwen3), so cloud reviewing them inline is cold w.r.t. the code — but cloud wrote the *spec*, so it is warm on the design. Therefore (refined 2026-06-24): **mechanical** code/test steps get cloud-inline review; **security-critical or complex** steps (egress/gate logic, ADK/model wiring, auth) ALSO get a **subagent** cold-review against the real repo; and the **whole feature gets one consolidated subagent cold-review before the PR**. This has repeatedly caught problems a single pass missed — Cycle 4 pitch (×2), Cycle 12 (×2; v1's egress claim was wrong), Scout pitch (the ADK-vs-`_call_cloud` contradiction that would have failed the capstone), Cycle 28.1 (the pitch's own write-site enumeration was wrong — 4 sites, not the claimed 2 — and a fail-closed/Librarian interaction would have silently converted corrupt-registry approvals into recorded rejections). One extra pass is cheap; a wrong locked decision is a rebuild.

**INVARIANT ENUMERATION RULE (added Cycle 28.1):** when a cycle changes a shared invariant that multiple call sites must honor (identity tagging, gate coverage, audit format, permission checks — anything one seam enforces on behalf of many callers), the pitch MUST contain the enumerated list of ALL call sites of that seam (paste the grep output into the doc, don't describe it from memory), and the cold review INDEPENDENTLY RE-RUNS the grep rather than trusting the pitch's list. Cycle 27 did this for all 11 egress sites and egress shipped clean; Cycle 28 didn't do it for the 3 write sites to `memory_identities` and 2 of them broke (the third was safe only by coincidence). The reviewer re-running the grep is the part that actually catches an undercount — a pitch that lists 2 sites reads just as confidently as one that correctly lists 4.

**REAL-SCHEMA TEST RULE (added Cycle 28.1):** any module that executes SQL gets at least one test against a real `init_schema()` database, not a monkeypatched connection. Monkeypatches can't catch schema drift — `observe.py` shipped querying a `sessions.updated_at` column that has never existed, at two call sites, silently mistagging every observation event ever recorded (`identity="personal", project=""`) for the life of the feature, because every test that touched it mocked the connection instead of exercising the real schema.
