...
### Session 10 — 2026-06-01 (Layer 4 revisit refinements)

**Done:**
- Layer 4 revisit completed (per user's Session 9 commitment to ensure nothing missed; Complete-over-fast principle applied)
- All 9 decisions (#61-#69) refined with additional locks; mechanism architecture unchanged; refinements clarify Phase-1 starting state and Stage-1 deferral specifics
- Walked decision-by-decision (Mode A) with second-pass prompts; user contributed substantive refinements throughout

**Key refinements per decision:**
- #61: Phased default chain explicit (Phase 1: NEU→Personal→Local with shadow-all + Testing Protocol feed; Phase 2+: per-class graduation via #68); cost ~2× local compute during Phase 1 accepted (Complete-over-fast)
- #62: Six classes confirmed (no taxonomy expansion); philosophy note added — local v1 = memory management + system-ctrl + graduated classes; cloud = reasoning lift
- #63: Cascade reframed as confidence-driven and phase-aware; step (2) try-local-first conditional on graduation per #68
- #64: Terminology sharpened (SUPPLIER ≠ MODEL ≠ ACCOUNT); curated POOL of ~5-10 models replaces arbitrary cycle limit; fail-closed message enumerates per-target reasons (Honesty)
- #65: Tier 3 includes irreversible-IN-EFFECT (broadcast actions); ROUND UP WHEN UNCERTAIN safety bias locked; classifier reliability is the linchpin (Stage 1 revisit path if insufficient)
- #66: Thresholds user-configurable; per-query HIGH-bump capability; terminate offers local-only completion
- #67: Chains DRAWN from pool (not hand-listed); retry=2 default; content-policy block surfaces to user (no auto-retry)
- #68: Success definition SHAPE = blended (user-acceptance + shadow-agreement); thresholds class-specific; un-graduation = rolling N=20 window
- #69: Hedged language until v1.0 ships; publication-strategy stub = README + blog post at v1.0 (paper deferred to research path #18.5)

**Substantive user contributions:**
- Account decoupled from identity reaffirmed; data protection comes from Layer 3e cascade not account-binding
- "No shortcuts" framing during Phase 1 — shadow-all preferred over sampling for cleaner reputation building
- Terminology correction: SUPPLIER = company offering models (Claude), MODEL = options within (Opus/Sonnet)
- Curated pool insight: top 5-10 models cover 95% of tasks; cycle limit becomes non-issue
- Round-up-when-uncertain safety bias for Safety Copilot classifier
- Layer 4 revisit confirms 6-of-9 layers locked; refinements clarify but do not restructure

**Side discussions:**
- Stage taxonomy clarified: only Stage 0 (blueprinting, current) and Stage 1 (implementation, next) formally locked; Phase 1/2/3 are within Stage 1, not separate stages; user chose to keep simple — not over-stage
- Personal Claude Pro cancellation discussed; user declined to lock a decision — will decide independently when timing feels right

**Layer 4 status: FULLY LOCKED with Session 10 refinements.** Three layers fully locked (3c, 3e, 4); three layers locked at directional altitude (3a, 3b, 3d); three layers remain to design (5, 6, 7 + 1+2 detail).

**Next session resume point:**
- Begin Layer 5 (Memory Storage) design (5-type schema locked #46; needs operational design: on-disk layout, SQLite vs files, retention, partition tag enforcement) — likely next
