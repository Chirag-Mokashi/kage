...
*Status: PITCH (cloud-authored plan, Opus 4.8, 2026-06-21). Built per the 7-step dev workflow: plan cloud → write local (Qwen3) → review cloud → plan tests cloud → write tests local → review tests cloud → run tests local.*
...
1. **ADK is the orchestrator, not `_call_cloud`.** The brainstorm said Scout "reuses `_call_cloud`." That would be a plain Python script and fails the capstone's ADK requirement. **Locked: Scout is a `SequentialAgent` of two `LlmAgent` stages**, model-routed via `LiteLlm` (Qwen3→Ollama, cloud→OpenRouter/Sonnet). The 4-pass design maps onto this cleanly: Pass 1+2 = local stage, Pass 3+4 = cloud stage.
...
- **`build_pipeline(cfg, *, cloud: bool) -> SequentialAgent`** — wires the two `LlmAgent` stages. `cloud=False` (bootstrap/dry-run) returns the broad stage only.
...
def run(mode: str) -> None:
    ...
    runner = InMemoryRunner(agent=pipeline, app_name="kage-scout")
...
> **Step 0 still applies:** if installed `google-adk` 2.3.0 differs on any of `session_service.create_session`, `run_async`, `event.is_final_response()`, or `types.Content`/`types.Part`, fix to match the installed surface — do not invent symbols.
...
| `test_build_pipeline_bootstrap_skips_cloud` | `cloud=False` returns a SequentialAgent with `len(sub_agents)==1` (broad only — no cloud egress in bootstrap) |
...
Target: ~13 tests. The gating tests (`test_scout_recall_gates_local_only`, `test_build_pipeline_bootstrap_skips_cloud`) are the **security-critical** ones — they prove no personal data leaves except through the gate.
...
|---|---|
| ADK / multi-agent | `SequentialAgent` of two `LlmAgent` stages, LiteLLM-routed |
