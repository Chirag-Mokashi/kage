...
calendar: NEU (mokashi.ch@northeastern.edu)
...
- **v2 → v3:** B1 resolved by live spike (full access not grantable) → scope cut to create-only. This is a *reduction* of the cold-reviewed create path (delete removed), so no new surface was introduced; cold review #1 covers the shipping v3 create path. Per the "1–2 cold reviews" rule, this is accepted at one review.
