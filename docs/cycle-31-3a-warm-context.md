...
*Status: PITCH v3 — BUILD-LOCKED. THREE cold reviews (subagent, Opus): #1 REVISE (3 blockers), #2 REVISE (verified B1 sound; caught the still-undercounted enumeration + non-implementable Librarian consumer + `kage use` gap), #3 APPROVE WITH EDITS ("the enumeration has closed; build-ready" — independently re-ran both enumerations across all modules, confirmed exactly 13 resolution sites + 3 injection sites, no 4th path; all v3 fixes SOUND; one trivial parenthetical edit applied). Ready to build via the 7-step gate.*
...
> - **NEW-4 (MINOR)** — `chat` (`cli.py:1806`) ignores sticky today (defaults `--identity personal`); wiring rich is a real semantic change. Fixed: **flagged as intentional** — `kage chat` will now respect `kage use` (surprising that it doesn't; this is a fix, stated).
...
- **`_resolve_context_rich() -> Resolution`** (dataclass: per-axis `value, confidence, provenance`) is called by exactly **3 warm consumers** (cold review #2 cut the Librarian — NEW-2):
  ...
  2. `cli.py:1806` — `chat` — **injection site NOT captured by the `_resolve_context` grep**; today defaults `--identity personal` and ignores sticky, so wiring rich is an **intentional behavior fix** (`kage chat` will now respect `kage use` — stated, not silent; NEW-4)
