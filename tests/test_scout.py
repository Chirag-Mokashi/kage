...
def _fake_scout_303_fixture(monkeypatch, tmp_path, report_text, deposit_fn=None):
    """Shared fixture for Cycle 30.3 deposit-visibility tests -- mirrors the
    existing test_run_two_stage_pipeline harness."""
    item = {"source": "hn", "title": "Good item", "url": "http://x", "score": 1, "snippet": "s"}
    ...
    monkeypatch.setattr(scout, "InMemoryRunner", make_runner)
    monkeypatch.setattr(scout, "_fetch_full", lambda it: "some article content")
    monkeypatch.setattr(scout.guard, "_scout_triage", lambda *a, **kw: False)
    monkeypatch.setattr(scout, "_write_report", lambda *a, **kw: None)
    monkeypatch.setattr(scout, "_update_cache", lambda *a, **kw: None)
    monkeypatch.setattr(scout, "_token_log", lambda *a, **kw: None)
    monkeypatch.setattr(scout, "_resolve_context", lambda *a: ("personal", "kage", None))
    if deposit_fn is not None:
        monkeypatch.setattr(scout, "deposit_to_queue", deposit_fn)


def test_run_deposits_recognized_headers_no_warning(monkeypatch, tmp_path, capsys):
    report_text = "## Tier 1\n### Card A\nbody a\n### Card B\nbody b\n## Tier 2\n- other"
    deposits = []
    def fake_deposit(content, source, **kw):
        deposits.append(content)
        return "some-id"
    _fake_scout_303_fixture(monkeypatch, tmp_path, report_text, fake_deposit)
    ...
    assert audit_calls == []
    assert "Scout parsed 0 cards" not in capsys.readouterr().out


def test_run_drifted_headers_zero_deposits_warns(monkeypatch, tmp_path, capsys):
    report_text = "**Tier 1**\n### Card A\nbody a\n**Tier 2**\n- other"
    def fake_deposit(content, source, **kw):
        return "some-id"
    _fake_scout_303_fixture(monkeypatch, tmp_path, report_text, fake_deposit)
    ...
    scout.run("run")
    flagged = [r for r in audit_calls if r.get("event") == "scout_deposit_empty"]
    assert len(flagged) == 1
    assert flagged[0]["report_chars"] == len(report_text)
    assert "Scout parsed 0 cards" in capsys.readouterr().out


def test_run_parse_exception_audits_partial_count(monkeypatch, tmp_path, capsys):
    report_text = "## Tier 1\n### Card A\nbody a\n### Card B\nbody b\n### Card C\nbody c\n## Tier 2\n- other"
    calls = []
    def flaky_deposit(content, source, **kw):
        calls.append(content)
        if len(calls) == 3:
            raise RuntimeError("boom")
        return "some-id"
    _fake_scout_303_fixture(monkeypatch, tmp_path, report_text, flaky_deposit)
    ...
    result = scout.run("run")
    assert result == report_text
    parse_errors = [r for r in audit_calls if r.get("event") == "scout_deposit_parse_error"]
    assert len(parse_errors) == 1
    assert parse_errors[0]["deposited_before_error"] == 2
    empty_flags = [r for r in audit_calls if r.get("event") == "scout_deposit_empty"]
    assert empty_flags == []
    assert "Scout parsed 0 cards" not in capsys.readouterr().out


def test_run_empty_report_no_false_warning(monkeypatch, tmp_path, capsys):
    report_text = ""
    _fake_scout_303_fixture(monkeypatch, tmp_path, report_text)
